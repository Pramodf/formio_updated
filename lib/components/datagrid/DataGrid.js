'use strict';
var isBlock = false, exlFlag = true;
...
var DataGridComponent = exports.DataGridComponent = function (_FormioComponents) {
  ...
  _createClass(DataGridComponent, [{
    ...
    value: function buildTable() {
      ...
      if (this.component.validate && this.component.validate.minLength > this.data[this.component.key].length) {
        ...
        for (var i = 0; i < toAdd; i++) {
          this.data[this.component.key].push({});
			...
			isBlock = this.component.disabled;
      this.tableRows = this.data[this.component.key].map(function (row, rowIndex) {
        return _this2.buildRow(row, rowIndex, isBlock);
      });
      ...
      if (addButton) {
        this.tableElement.appendChild(addButton);
			...
			// if (this.component.isEdit === true) {
			// 	$('.glyphicon-remove-circle').closest('button').hide();
			// }
    }
    ...
    key: 'buildRows',
    value: function buildRows(data, hasLinkAnnotation, addMoreFlag) {
      var _this4 = this;
		  this.data[this.component.key].forEach(function (row, rowIndex) {
        // New Row.
        if (!_this4.tableRows[rowIndex]) {
          _this4.tableRows[rowIndex] = _this4.buildRow(row, rowIndex, isBlock);
          _this4.tbody.insertBefore(_this4.tableRows[rowIndex], _this4.tbody.children[rowIndex + 1]);
        }
        // Update existing
        else if ((hasLinkAnnotation) || !_lodash2.default.isEqual(row, _this4.tableRows[rowIndex].data) || !_lodash2.default.isEqual(_this4.visibleColumns, _this4.tableRows[rowIndex].visibleColumns)) {
            _this4.removeRowComponents(rowIndex);
						var newRow = _this4.buildRow(row, rowIndex, isBlock);
    ...
    key: 'buildRow',
    value: function buildRow(row, index, isBlock) {
      var _this5 = this;
			var adminCount;
      this.rows[index] = {};
      var element = this.ce('tr', null, [this.component.components.map(function (col, colIndex) {
				col.disabled = isBlock;
				var updateColumn = _.map(col.components, function(eleObj) {
					if (isBlock) {
						if (!eleObj.dateRule) {
							eleObj.disabled = isBlock;
						}
						eleObj.validate = {"required": false};
					}
					return eleObj;
				});
				col.components = updateColumn;
				adminCount = (col.components[0].adminCount) ? col.components[0].adminCount : 0;
        return _this5.buildComponent(col, colIndex, row, index);
      }), (!this.shouldDisable && index !== 0 ) ? this.ce('td', null, this.removeButton(index, '', '', adminCount, true)) : null]);
    ...
    value: function buildComponent(col, colIndex, row, rowIndex) {
      ...
      for(var i =0 ; i < column.components.length; i++) {
        if(column.components[i].adminCount !== 0 && ((column.components[i].adminCount - 1 ) >= rowIndex)) {
          column.components[i].validate = null;
        }
      }
    ...
    value: function checkConditions(data) {
      ...
      if (rebuild) {
        this.buildRows();
				...
				if (this.component.isEdit === true && this.tableElement.lastChild.firstChild) {
					$(this.tableElement.lastChild.firstChild).remove();
				} else {
					// Logic for add more button while default value is mandatory or optional
					if (!this.component.isEdit) {
            var filteredNames = filtered_keys(data, /(_EXCLSN_FLG_MNDTRY|_EXCLSN_FLG_OPTNL)/);
            var isOptionalKey = filteredNames.findIndex(function(key) {
              var optionalRegex =  /(_EXCLSN_FLG_OPTNL)/;
              return optionalRegex.test(key);
            });
            var isMandatoryKey = filteredNames.findIndex(function(key) {
              var mandatoryRegex =  /(_EXCLSN_FLG_MNDTRY)/;
              return mandatoryRegex.test(key);
            })
            var isNumeratorCondition = (isOptionalKey !== -1 ? (data[filteredNames[isOptionalKey]]  === false) : true) && 
            (isMandatoryKey !== -1 ? (data[filteredNames[isMandatoryKey]] === false) : true);

							if ((isOptionalKey !== -1 && data[filteredNames[isOptionalKey]] === true) && this.component.optional === false) {
								if (this.tableElement.lastChild.firstChild) {
									$(this.tableElement.lastChild.firstChild).remove();
									exlFlag = false;
								}
								// $('.glyphicon-remove-circle').closest('button').hide();
							} else if ((isMandatoryKey !== -1 && data[filteredNames[isMandatoryKey]]  === true) && this.component.mandatory === false) {
								if (this.tableElement.lastChild.firstChild) {
									$(this.tableElement.lastChild.firstChild).remove();
									exlFlag = false;
								}
								// $('.glyphicon-remove-circle').closest('button').hide();
							} else if (isNumeratorCondition) {
								if (this.component.mandatory === false && this.component.optional === false) {
									this.tableElement.lastChild.innerHTML = '';
									this.tableElement.lastChild.appendChild(this.ce('tr', null, this.ce('td', {
										colspan: this.component.components.length + 1
									}, this.addButton())));
								// $('.glyphicon-remove-circle').closest('button').show();
								} else {
									if (this.tableElement.lastChild.firstChild) {
										$(this.tableElement.lastChild.firstChild).remove();
										exlFlag = false;
