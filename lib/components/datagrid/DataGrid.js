				...
				if (this.component.isEdit === true && this.tableElement.lastChild.firstChild) {
					...
					if (!this.component.isEdit) {
						var filteredNames = filtered_keys(data, /(_EXCLSN_FLG_MNDTRY|_EXCLSN_FLG_OPTNL)/);
							if (data[filteredNames[0]] === true && this.component.optional === false) {
								if (this.tableElement.lastChild.firstChild) {
									this.tableElement.lastChild.firstChild.remove();
									exlFlag = false;
								}
								// $('.glyphicon-remove-circle').closest('button').hide();
							} else if (data[filteredNames[1]]  === true && this.component.mandatory === false) {
								if (this.tableElement.lastChild.firstChild) {
									this.tableElement.lastChild.firstChild.remove();
									exlFlag = false;
								}
								// $('.glyphicon-remove-circle').closest('button').hide();
							} else if (data[filteredNames[1]]  === false && data[filteredNames[0]] === false) {
