...
var BaseComponent = function () {
  ...
  _createClass(BaseComponent, [{
     ...
     *   key: 'firstName'
     * });
     * component.on('componentChange', function(changed) {
    ...
    value: function build() {
      if (this.viewOnly) {
        ...
        if(!this.component.isBlock && !this.component.multiple && this.component.adminCount > 0) {
          this.component.validate = null;
        }
        ...
        window.chartNavCompRef.component.setValidationClass();  
      }
    }
	}, 
	{
        ...
        var measureMandaotoryKey = window.chartNavCompRef.component.measure + '_EXCLSN_FLG_MNDTRY';

        var measureOptionalKey = window.chartNavCompRef.component.measure + '_EXCLSN_FLG_OPTNL';
    ...
    value: function buildRows() {
      ...
      _lodash2.default.each(_this.data[_this.component.key], function(value, index) {
				if (!addMoreFlag) {
					totalCnt = _this.data[_this.component.key].length;
				}
        var tr = _this.ce('tr');
        var td = _this.ce('td');
        ...
        if (!_this.shouldDisable) {
					...
					if ( index !== 0 && (index >= _this.component.adminCount)) {
						tdAdd.appendChild(_this.removeButton(index, true, entireobj.component.key));
					}
					var tdBtn = _this.ce('td');
					if ( index === (_this.data[_this.component.key].length - 1)) {
						tdBtn.appendChild(_this.addButton());
					}
      ...
      var removeButton = this.ce('button', {
        ...
        tabindex: '-1'
          ...
          window.chartNavCompRef.component.onRemoveFormComponent(index,IsMultiple,removekey);
            ...
            if(!IsMultiple && index === (_this3.data[datagrid.id].length - 1)) {
              window.chartNavCompRef.component.isBlockRemoved = true;
            }
				}
				// workaround to fix issue of date data loss in block delete 
				var processedArray = {};
				if(_this3.data && _this3.data.minMaxVal) {
					$.each(_this3.data.minMaxVal, function(i, n) {
						if(n[ index + "-"+ 0]) {
							delete n[ index + "-"+ 0];	
						} 
						var tempData = [], k = 0;
  ...
  {
    key: 'dateUpdateRangeCalculation',
    value: function dateUpdateRangeCalculation(container, component) {
      component = component || this.component;
      if (this.IsAdminBlock(component, 0)) {
        return;
      }
      if (!this.component.onChange || !this.component.onChange.action || this.component.onChange.action.toLowerCase() !== 'viewinfo' || !window.chartNavCompRef || !window.chartNavCompRef.component) {
        return;
      }

      this.buttonCalulate = this.ce('BUTTON', {
				class: 'calculate-Btn' 
      });

      this.buttonCalulate.innerHTML = window.chartNavCompRef.component.CONSTANT.calculateDateRangeBtnCaption;
      
      this.addEventListener(this.buttonCalulate, 'click', function (event) {
				window.chartNavCompRef.component.calculateDateRange(component.key);
      });
      
      container.appendChild(this.text(' '));
			
			container.appendChild(this.buttonCalulate);
    }
    ...
    value: function addInputGroup(input, container) {
      ...
      if (this.component.prefix || this.component.suffix) {
				if (this.component.linkAnnotation && container.tagName === 'TD') {
					//custom requirement to add annotation buttons beside field which has propert linkAnnotation: true
          if(this.component.multiple && this.component.adminCount > 0) {
            inputGroup = this.ce('div', {
              class: 'input-group displayInlineFlex  remove-error remove-validation'
            });
          } else {
            inputGroup = this.ce('div', {
          ...
          if(this.component.multiple && this.component.adminCount > 0) {
            inputGroup = this.ce('div', {
              class: 'input-group remove-error remove-validation'
            });
          } else {
            inputGroup = this.ce('div', {
              class: 'input-group'
    ...
    value: function redraw() {
      ...
      // start
      var isSubmitClickedOnAbstractnScreen = localStorage.getItem("submitBtnClickedByUserOnAbstractn");  
      var isSubmit = false;
      if(isSubmitClickedOnAbstractnScreen == "true"){
        isSubmit =  true;
      }
      //end
      // validation is on & component has 'Please Select' as value then don't rebuild
      if (window.chartNavCompRef.component.isValidationOn
        && isSubmit      
        && this.data && this.data.hasOwnProperty(this.component.key) 
    ...
    value: function fieldLogic(data) {
      ...
      var changed = logics.reduce(function (changed, logic) {
        ...
        if (this.component.mandatory === false && this.component.optional === false) {
          this.component.validate.required = true;
        }
		if(window.chartNavCompRef.component.isValidationOn) {
			$('div#' + this.component.id).children().toggleClass('removeContent-after-mandatory');
		}
				changed = true;
        ...
        this.component.validate.required = false;
        $('div#' + this.component.id).children().addClass('removeContent-after-mandatory');
				changed = true;
			}
    ...
    value: function setCustomValidity(message, dirty) {
      ...
      if (message) {
        ...
        if (this.component.type == 'checkbox' && this.component.optional == false && this.component.mandatory == false) {
			var errorClass = 'formio-errors invalid-feedback';
			if (!window.chartNavCompRef.component.isValidationOn) {
				errorClass = 'formio-errors';
			}
			var ele = '<div class="' + errorClass + '"><p class="help-block">' + message + '</p></div>';
          $('#' + this.errorContainer.id).children().remove(".formio-errors");
          $('#' + this.errorContainer.id).append(ele);
        }
