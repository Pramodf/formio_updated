'use strict';
var datagrid, addMoreFlag = false, isAdmin = false, totalCnt = 0 , changedField = '', currentFormData = null;
        ...
        tabindex: '-1'
			...
			this.addEventListener(removeButton, 'click', function (event) {
			  event.preventDefault();
				...
				// annotation realted change To avoid shuffling delete row which has already updated with annotation acons
    ...
    value: function createAnnotationButtons(container, component, classes,rowindex) {
      ...
      if (this.IsAdminBlock(component, rowindex)) {
        ...
        class: markClearBtnClass
			...
			if(currentFormData) {
				var measureMandaotoryKey = component.key.split('_')[0] + '_EXCLSN_FLG_MNDTRY';
				var measureOptionalKey = component.key.split('_')[0] + '_EXCLSN_FLG_OPTNL';
				if(currentFormData.hasOwnProperty(measureMandaotoryKey) && currentFormData[measureMandaotoryKey] === true && !component.mandatory) {
					isDisabled = true;
				} else if(currentFormData.hasOwnProperty(measureOptionalKey) && currentFormData[measureOptionalKey] === true && !component.optional) {
					isDisabled = true;
				} else if( (currentFormData.hasOwnProperty(measureMandaotoryKey) && currentFormData[measureMandaotoryKey] === false && component.mandatory)
									 || (currentFormData.hasOwnProperty(measureOptionalKey) && currentFormData[measureOptionalKey] === false && component.optional)) {
					isDisabled = true;
				}
			}
