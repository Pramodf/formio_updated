	...
	},
	{
		...
		value: function hideShowBlockDeleteBtns() {
			...
			if (formData) {
				...
				if((formData.hasOwnProperty(measureMandaotoryKey) && (formData[measureMandaotoryKey] === null || formData[measureMandaotoryKey] === false) 
				...
				if(_this3.data && _this3.data.minMaxVal) {
					$.each(_this3.data.minMaxVal, function(i, n){
						...
						if(n[ index + "-"+ 0]) {
							...
							//   if(count === index) {
							//     k.replace(index + 1 , index);
							//   } 
							//   count = count + 1;
							// });
								// Object.keys(n).forEach((key,ind) => {
								//  if(ind === index) {
								//    key.replace("2-0","1-0");
								//  } else if(ind > index) {
								//    key.replace((ind + 1).toString()  , ind.toString() );
								//  }
								// });
