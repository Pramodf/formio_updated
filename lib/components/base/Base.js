'use strict';
var datagrid, addMoreFlag = false, isAdmin = false, totalCnt = 0;;
  ...
  _createClass(BaseComponent, [{
      ...
      if (this.viewOnly) {
        ...
        if (this.shouldDisable) {
          this.disabled = true;
				...
				if (this.component.multiple == false) {
    ...
    value: function buildRows() {
      ...
      this.inputs = [];
			...
			var entireobj= this;
			if (this.component.sourceType === 'ADMIN') {
				isAdmin = true;
			}		
      ...
      _lodash2.default.each(this.data[this.component.key], (value, index) => {
        ...
        if (!_this.shouldDisable) {
					...
					if ( index !== 0 && (isAdmin && index >= totalCnt)) {
      ...
      var removeButton = this.ce('button', {
        ...
        tabindex: '-1'
			...
			this.addEventListener(removeButton, 'click', function (event) {
			  event.preventDefault();
				if (window.chartNavCompRef && window.chartNavCompRef.component) {
					window.chartNavCompRef.component.onRemoveFormComponent(index,IsMultiple,key);
				}
				// To avoid delete row which has already updated with annotation acons
				if (window.chartNavCompRef && window.chartNavCompRef.component && window.chartNavCompRef.component.hasAnnotationLink && !IsMultiple){
    ...
    value: function IsAdminData(container, component, index) {
			...
			if (addMoreFlag === false && component.sourceType === 'ADMIN' && (component.isBlock || component.type !== 'datetime')) {
    ...
    value: function createAnnotationButtons(container, component, classes,rowindex) {
      ...
      if (!this.IsLinkAnnotationComponent()) {
        ...
        class: 'gotoAnnotationBtn'
			...
			if (window.chartNavCompRef.component.annotationKeyValObj != undefined) {
				var obj = window.chartNavCompRef.component.annotationKeyValObj.Keys;
				if(obj[component.key] !== undefined) {
					var arrobj = obj[component.key].split('|');
					var indexid = component.multiple ? rowindex : component.row.split('-')[0];
					if(arrobj.length >= indexid ) {
						if(arrobj[indexid] != '')
							markClearBtnClass = 'clearAnnotationBtn';
						 else
						  this.buttonGoAnnot.disabled = true;
					}
				}
			}
        ...
        class: markClearBtnClass
			...
			this.addEventListener(this.buttonGoAnnot, 'click', function (event) {
				var rowID = component.multiple ? rowindex : component.row.split('-')[0];
			...
			this.addEventListener(this.buttonMarkAnnot, 'click', function (event) {
				...
				window.chartNavCompRef.component.setAnnotationKey(Number(rowID) + '-' + component.key,$(this),action);
				if(action == 'clear') {
					$(this).closest(':has(button.gotoAnnotationBtn)').find('button.gotoAnnotationBtn').attr("disabled", "disabled");
				}
			});
    ...
    value: function addInputGroup(input, container) {
      ...
      if (this.component.prefix || this.component.suffix) {
				if (this.component.linkAnnotation) {
					//custom requirement to add annotation buttons beside field which has propert linkAnnotation: true
    ...
    value: function fieldLogic(data) {
      ...
      var newComponent = _lodash2.default.cloneDeep(this.originalComponent);
			var resultval;
			var isCheckboxEnableDisableActionType = false;
      var changed = logics.reduce(function (changed, logic) {
              ...
              case 'value':
                {
                  ...
                  break;
							...
							case 'CheckboxEnableDisable':
							isCheckboxEnableDisableActionType =  true;
                // TODO
                break;
            }
            return changed;
					...
					logic.actions.reduce(function (changed, action) { 
						if(action.type === 'CheckboxEnableDisable'){
							isCheckboxEnableDisableActionType =  true;
						}
					});
				}
        return changed;
			...
			// checkbox enable/disabled case handling
			if(isCheckboxEnableDisableActionType && resultval === false && this.originalComponent.type === "checkbox" && this.component.disabled === true  
					&& this !== undefined && this.originalComponent !== undefined){
				this.component.disabled = false;
				changed = true;
			} else  if(isCheckboxEnableDisableActionType && resultval === true && this.originalComponent.type === "checkbox" && this.originalComponent.disabled === false  
			&& this !== undefined && this.originalComponent !== undefined){
