    ...
    value: function setSubmission(submission) {
      var _this7 = this;
			if(window.chartNavCompRef.component.isNavigatedMeasure && 
				  window.chartNavCompRef.component.memData.measureName === window.chartNavCompRef.component.prevMeasureName){
				        _this7.data = window.chartNavCompRef.component.submission.data;
			}
      ...
      return new _nativePromiseOnly2.default(function (resolve, reject) {
        ...
        _this11.hook('beforeSubmit', submission, function (err) {
          ...
          var hasSameDateInBlock = false;
            ...
            // to check multiple block has not same date
            const blockData = Object.keys(submission.data).filter(function (propertyName) {
              return propertyName.indexOf('BLOCK') === 0;
            });

            if(blockData.length > 0) {
              if(submission.data[blockData[0]]) {
                var firstDateFieldKey = window.chartNavCompRef.component.blockDateCompareFieldKeyName;
                if(firstDateFieldKey !== '' && submission.data[blockData[0]]) {
                  for(var j = 0; j < submission.data[blockData[0]].length; j++) {
                    var dateValue  = submission.data[blockData[0]][j][firstDateFieldKey];
                    if(dateValue) {
                      for(var k = 0; k < submission.data[blockData[0]].length; k++) {
                        if(j !== k) {
                          if(submission.data[blockData[0]][k][firstDateFieldKey] === dateValue) {
                            hasSameDateInBlock = true;
                            break;
                          }
                        }
                      }
                    }
                    if(hasSameDateInBlock) {
                      break;
                    } 
                  }
                }
								...
								if(error[0].component.row === "")  {
									...
									try {
											...
											if(blockItems.length > 0) {
												...
												if(blockName === "") {
													_this11.showErrors();
												} else if(submission.data[blockName] && (parseInt(errorBlockIndex) >= submission.data[blockName].length)) {
													isFormFieldsValid = true;
												}
											}
										} catch(e) {
          ...
          if (submission && submission.data && isMandatoryAnnotationValid && isFormFieldsValid && !hasSameDateInBlock) {
            ...
            if(hasSameDateInBlock) {
              $('.blockSameDateErrorMessage').addClass('alert-danger');
              requiredElement = $('.blockSameDateErrorMessage');
            } else if(elements.length > 0 && hasElement){
