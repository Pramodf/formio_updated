...
var FormioForm = function (_FormioComponents) {
  ...
  _createClass(FormioForm, [{
    ...
    value: function setSubmission(submission) { 
      var _this7 = this;
			if((window.chartNavCompRef.component.isNavigatedMeasure)
          || window.chartNavCompRef.component.absService.isMemberChange()){
                if(_.isEqual(_this7.data, submission.data)) {

                 } else {
                  _this7.data = submission.data; 
                }
			}
        ...
        if (!_this7.setValue(submission)) {
          _this7.triggerChange({
            noValidate: true
				...
				window.chartNavCompRef.component.isNavigatedMeasure = false;
    ...
    value: function onChange(flags, changed) {
      if (typeof value == "undefined" && typeof changed == "undefined") {
        if (performanceFlag > 5) {
          performanceFlag = 0;
          return true;
        } else {
          if (flags) 
    ...
    key: 'groupByKey',
    value: function groupByKey(arr, key) {
      var groupComponents = [];
      var components = {};
      var i;
      var j;
      ...
      for (var i = 0, j = arr.length; i < j; i++) {
        cur = arr[i].component;
          if (!(cur[key] in components)) {
            components[cur[key]] = { Id: cur[key], data: [] };
            groupComponents.push(components[cur[key]]);
          }
          components[cur[key]].data.push(cur);
      }
    ...
    value: function executeSubmit() {
      var _this11 = this;
      // Fix for Submit button disable issue if dropdwn has default select value
      // start
      localStorage.setItem("submitBtnClickedByUserOnAbstractn", true);
      // end
        ...
        _this11.hook('beforeSubmit', submission, function (err) {
          if (err) {
            ...
            return reject(err.message || err);
					...
					var isRejectAction = window.chartNavCompRef.component.isRejectButtonAction();
					var isMandatoryAnnotationValid = false;
					var isFormFieldsValid = false;
          var isOverreadCommentEmpty = false;
					if(isRejectAction) {
						 if(submission.data.OVER_READ_COMMENT === '' ) {
							isOverreadCommentEmpty = true;
						 } else {
							isMandatoryAnnotationValid = true;
							isFormFieldsValid = true;
						 }
					} else {
            var isValidationOn = window.chartNavCompRef.component.isValidationOn;
            /**
             *  MRR-9100: Global Client specific annotation validation on/off feature
             *  configuartion is manageable via global.ts (IsAnnotationValidationOn)
             */
            var isAnnotationValidationOn = window.chartNavCompRef.component.GLOBALS.IsAnnotationValidationOn;
            if (isAnnotationValidationOn && isValidationOn) { 
              // only show annotation errors when anotation validation is ON and form validation is also ON
              isMandatoryAnnotationValid = _this11.validateMandatoryAnnotation(submission.data);
            } else {
              isMandatoryAnnotationValid = true;
            }
            // Abstarction form validation on/off

            if(isValidationOn) {
                isFormFieldsValid = _this11.checkValidity(submission.data, true);
                
                // work around to fix Disable submit issue- in block delete & submit
                if (isFormFieldsValid === 0 && window.chartNavCompRef.component.isBlockRemoved) {
                  try {
                  var  error = _this11.errors;
                  if(error instanceof Array && error.length > 0) {
                    var gropedError = _this11.groupByKey(error , 'id');
                    var validCount = 0;
                    for( var  j = 0; j < gropedError.length; j++) {
                      var componentID = gropedError[j].Id;
                      var objectValidation = [];
                      var isDataValid = false;
                      var blockItems = filtered_keys(submission.data, /(BLOCK)/);
                      if(blockItems.length > 0) {
                        $.each( blockItems , function( key, value ) {
                          if(submission.data[value][0].hasOwnProperty(componentID)) {

                            if(_this11.type === 'form') {
                              for(var k = 0 ; k < _this11.component.components.length; k++) {
                                  if(_this11.component.components[k].id === value) {
                                      for(var i = 0; i < _this11.component.components[k].components[0].components.length; i++) {
                                        var validationObj = { isRequired: false , minVal: null , maxVal: null , pattern: null,
                                        adminCount: _this11.component.components[k].components[0].components[i].adminCount };

                                        if(_this11.component.components[k].components[0].components[i].id === componentID && 
                                          _this11.component.components[k].components[0].components[i]["validate"])
                                        {
                                          if(_this11.component.components[k].components[0].components[i]["validate"].hasOwnProperty("required")
                                            && _this11.component.components[k].components[0].components[i]["validate"]["required"] === true)
                                          {
                                            validationObj.isRequired = true;
                                          }
                                          if(_this11.component.components[k].components[0].components[i]["validate"].hasOwnProperty("min")
                                          && _this11.component.components[k].components[0].components[i]["validate"]["min"] !== null)
                                          {
                                            validationObj.minVal = _this11.component.components[k].components[0].components[i]["validate"]["min"];
                                          }

                                          if(_this11.component.components[k].components[0].components[i]["validate"].hasOwnProperty("max")
                                          && _this11.component.components[k].components[0].components[i]["validate"]["max"] !== null)
                                          {
                                            validationObj.maxVal = _this11.component.components[k].components[0].components[i]["validate"]["max"];
                                          }

                                          if(_this11.component.components[k].components[0].components[i]["validate"].hasOwnProperty("pattern")
                                          && _this11.component.components[k].components[0].components[i]["validate"]["pattern"] !== null)
                                          {
                                            validationObj.pattern = _this11.component.components[k].components[0].components[i]["validate"]["pattern"];
                                          }
                                          objectValidation.push(validationObj);
                                          break;
                                        }
                                    }
                                    break;
                            ...
                            innerloop: for( var l = 0; l < submission.data[value].length; l++) {
                                var fieldval = submission.data[value][l][componentID];
                              if(objectValidation[0].adminCount !== 0 && (objectValidation[0].adminCount-1) >= l)
                              {
                                isDataValid = true;
                              } else {
                                // check required validation
                                if(objectValidation[0].isRequired && fieldval !== null && fieldval !== "" && fieldval !== ' ') {
                                  isDataValid = true;
                                } else if(objectValidation[0].isRequired === false) {
                                  isDataValid = true;
                                } else if(objectValidation[0].isRequired && (fieldval === null || fieldval === "" || fieldval === " ")) {
                                  isDataValid = false;
                                  break innerloop;
                                }
                                ...
                                if(objectValidation[0].minVal !== null) {
                                  if(fieldval >= objectValidation[0].minVal) {
                                    isDataValid = true;
                                  } else {
                                    isDataValid = false;
                                    break innerloop;
                                ...
                                if(objectValidation[0].maxVal !== null) {
                                  if(fieldval <= objectValidation[0].maxVal) {
                                    isDataValid = true;
                                  } else {
                                    isDataValid = false;
                                    break innerloop;
                                ...
                                // if(objectValidation[0].pattern !== null) {
                                //   var pattern =  new RegExp(objectValidation[0].pattern);
                    ...
                    if(gropedError.length === validCount) {
                      isFormFieldsValid = true;
                    }
                  }
                } catch(e) {
              ...
              } else if (isFormFieldsValid === 0 && window.chartNavCompRef.component.isSingleFieldMultipleBlockAdmin) {
                var  error = _this11.errors;
                if (error instanceof Array && error.length > 0) {
                  var gropedError = _this11.groupByKey(error , 'id');
                  var validCount = 0;
                    for( var  j = 0; j < gropedError.length; j++) {
                      var componentID = gropedError[j].Id;
                      var isDataValid = false;
                      if (gropedError[j].data[0].multiple && gropedError[j].data[0].adminCount > 0 && gropedError[j].data[0]["validate"]) {
                        var componentData = submission.data[componentID];
                        var componentValidation = gropedError[j].data[0]["validate"];
                        for(var a = 0; a < componentData.length; a++) {
                          var fieldval = componentData[a];
                          // check required validation
                          if(gropedError[j].data[0].adminCount !== 0 && (gropedError[j].data[0].adminCount - 1) >= a)
                          {
                            isDataValid = true;
                          } else {
                            if(componentValidation.hasOwnProperty("required")) {
                              if(componentValidation.required && fieldval !== null && fieldval !== "") {
                                isDataValid = true;
                              } else if(componentValidation.required === false) {
                                isDataValid = true;
                              } else if(componentValidation.required && (fieldval === null || fieldval === "")) {
                                isDataValid = false;
                                break;
                              }
                            }
                            ...
                            // check min validation
                            if(componentValidation.hasOwnProperty("min")) {
                              if(componentValidation.min !== null) {
                                if(fieldval >= componentValidation.min) {
                                  isDataValid = true;
                                } else {
                                  isDataValid = false;
                                  break;
                            ...
                            // check max validation
                            if(componentValidation.hasOwnProperty("max")) {
                              if(componentValidation.max !== null) {
                                if(fieldval <= componentValidation.max) {
                                  isDataValid = true;
                                } else {
                                  isDataValid = false;
                                  break;
          ...
          if(isValidationOn === false && window.chartNavCompRef.component.overReadUserAccess && submission.data.OVER_READ_COMMENT.trim() === '' ) {
            isOverreadCommentEmpty = true;
            isFormFieldsValid = false;
          }

          if (submission && submission.data && isMandatoryAnnotationValid && isFormFieldsValid) {
            _this11.loading = true;
            localStorage.removeItem("submitBtnClickedByUserOnAbstractn");
            ...
            $("#OVER_READ_COMMENT_ERROR_MSG").remove();
						if(!isOverreadCommentEmpty) {
							_this11.showErrors();
						} else {
              $('#OVER_READ_COMMENT textarea').addClass('is-invalid');
              $("#OVER_READ_COMMENT").children().remove(".invalid-feedback");
              $('#OVER_READ_COMMENT')
                .append('<div id="OVER_READ_COMMENT_ERROR_MSG" class="formio-errors invalid-feedback"><p class="help-block">Comments are required</p></div>');
						}
            
						var requiredElement = $('.formio-form').children().first().children('div').first();
						// var firstElementID = $('.formio-form').children().first().children('div').first().attr("id");
            ...
            if(elements.length > 0 && hasElement){
							requiredElement = elements[indexOfElement];
						} else {
							var elements = document.getElementsByClassName('mandatoryError');
            ...
            $('.remove-error').addClass('remove-validation');
	...
	{
    key: 'validateMandatoryAnnotation',
    value: function validateMandatoryAnnotation(submitData) {
      var invalidAnnotation = null;
      if(window.chartNavCompRef.component.isValidationOn) {
        invalidAnnotation = window.chartNavCompRef.component.getInvalidMandatoryAnnotation(submitData);
      } else if(window.chartNavCompRef.component.isAnnotationRequiredOnValidationOff){
        invalidAnnotation = window.chartNavCompRef.component.getMandatoryAnnotationOnValidationOff(submitData);
      } else {
        return true;
      }
			var hasAnnotationError = false;
			if(invalidAnnotation && invalidAnnotation.length > 0) {
				for(var i = 0; i < invalidAnnotation.length; i++) {
					var id = '#' + invalidAnnotation[i].keyName + ' button.markAnnotationBtn';
          var isElementPresent = ($(id).length > 0);
          
          if(window.chartNavCompRef.component.isValidationOn) {
            $(id).addClass('mandatoryError');
          } else {
              if($('#' + invalidAnnotation[i].keyName + ' table').length > 0)
              { 
                $('#' + invalidAnnotation[i].keyName + ' > table > tbody  > tr').each(function() {
                 var fieldInput =  $(this).find(':input')[0];
                 if(fieldInput.value !== undefined && fieldInput.value !== null && fieldInput.value !== '') {
                    $(this).find('button.markAnnotationBtn').addClass('mandatoryError');
                 }
                });
              } else {
                if($(id).length > 1) {
                  $(id).each(function(value) {
                    var parent = this;
                    var fieldInput = $(parent.parentElement.parentElement).find(':input')[0];
                    if(fieldInput.value !== undefined && fieldInput.value !== null && fieldInput.value !== '') {
                      $(parent).addClass('mandatoryError');
                    }
                  })
                } else {
                  $(id).addClass('mandatoryError');
					...
					const hiddenattr = $('#' + invalidAnnotation[i].keyName).attr('hidden');
					// For some browsers, `attr` is undefined; for others,
					// `attr` is false.  Check for both.
					if (!isElementPresent || (isElementPresent && hiddenattr == "hidden" && !invalidAnnotation[i].isConditional)) {
      ...
      if (!before) {
        return this.beforeSubmit().then(function () {
          return _this12.executeSubmit();
					...
					console.log('Validation Error Occurred : ' + JSON.stringify(error))
					return false;
				});
      } else {
